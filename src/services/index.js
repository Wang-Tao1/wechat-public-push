  ...
  birthdayList.forEach((item, index) => {
    if (
      ...
      if (item.type === '生日') {
        ...
        console.log(getLB(),888);
        if (item.diffDay === 0) {
          birthMessage += `👑今天是 ${item.name} 的${age && item.isShowAge ? `${(item.useLunar ? 1 : 0) + age}岁` : ''}生日哦，祝${item.name}生日快乐！${getLB()}`
        } else {
          birthMessage += `❤️距离 ${item.name} 的${age && item.isShowAge ? `${age + 1}岁` : ''}生日还有${item.diffDay}天${getLB()}`
        }
      }
      ...
      if (item.type === '节日') {
        if (item.diffDay === 0) {
          festivalMessage += `🌸今天是 ${item.name} 哦，要开心！${getLB()}`
        } else {
          festivalMessage += `🎈距离 ${item.name} 还有${item.diffDay}天 ${getLB()}`
        }
      }
...
export const getAggregatedData = async () => {
  ...
  for (const user of users) {
    ...
    const wxTemplateParams = [
      { name: toLowerLine('desc'), value: user.desc, color: user.descColor || getColor() },
      ...
      { name: toLowerLine('city'), value: user.city || config.CITY, color: user.cityColor },
